...
# team. Copyright (c) 2018, NVIDIA CORPORATION.  All rights reserved.
#
...
BERT and RoBERTa are fine-tuned
...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str,
        ...
        cached_features_file = os.path.join(
            directory, args.model_type + "_cached_lm_" + str(block_size) + "_"
                       + filename
        )
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            # sake of simplicity (no padding)
            # If your dataset is small, first you should loook for a bigger
            # one :-) and second you
...
def _rotate_checkpoints(args, checkpoint_prefix="checkpoint",
    ...
    for checkpoint in checkpoints_to_be_deleted:
        logger.info("Deleting older checkpoint [{}] due to "
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args) \
    ...
    if tokenizer.mask_token is None:
        raise ValueError(
            ...
            "masked language modeling. Remove the --mlm flag if you want to "
            "use this tokenizer."
        )
...
def train(args, train_dataset, model: PreTrainedModel,
    ...
    model = model.module if hasattr(model, "module") else model  # Take care
    # of distributed/parallel training
    ...
    for i, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.n_gpu > 1:
                loss = loss.mean()  # mean() to average on multi-gpu parallel
                # training
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The model checkpoint for weights initialization. Leave None if "
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Optional pretrained tokenizer name or path if not the same "
             "as model_name_or_path. If both are None, initialize a new "
             "tokenizer.",
    )
    parser.add_argument(
        "--cache_dir",
    ...
    if args.should_continue:
        ...
        if len(sorted_checkpoints) == 0:
            raise ValueError("Used --should_continue but no checkpoint was "
                             "found in --output_dir.")
        else:
    ...
    else:  # Initializes the distributed backend which will take care of
    ...
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Barrier to make sure only the first
    ...
    if args.config_name:
        ...
        raise ValueError(
            "You are instantiating a new config instance from scratch. This is "
            "not supported, but you can do it from another script, save it,"
    ...
    for param in model.named_parameters():
        if 'bert.encoder.layer' in param[0]:
            if '8' not in param[0] or '9' not in param[0], '10' not in param[0], '11' not in param[0]:
                param[1].requires_grad = False
    ...
    if args.local_rank == 0:
        torch.distributed.barrier()  # End of barrier to make sure only the
        # first process in distributed training download model & vocab
    ...
    if args.do_train:
        if args.local_rank not in [-1, 0]:
            torch.distributed.barrier()  # Barrier to make sure only the first
            # process in distributed training process the dataset, and the
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            global_step = checkpoint.split("-")[-1] if len(checkpoints) > 1 \
                else ""
